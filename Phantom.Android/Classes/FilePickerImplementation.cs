...
namespace Phantom.Droid.Classes
{
    ...
    {
        public Task<PhantomImageData> PickImageAsync()
        {
            ...
            activity.PickImageTaskCompletionSource = new TaskCompletionSource<PhantomImageData>();

            // Return Task object
            return activity.PickImageTaskCompletionSource.Task;
        }

        public async Task<PhantomImageData> PickFileAsync()
        {
            FileResult fileData = await FilePicker.PickAsync();
            if (fileData == null)
                return null; // User canceled file picking

            PhantomImageData Phantom_img_data = new PhantomImageData() { name = fileData.FileName, path = fileData.FullPath, stream = await fileData.OpenReadAsync() };

            // Return Task object
            return Phantom_img_data;
        }
