...
[assembly: ExportRenderer(typeof(Xamarin.Forms.WebView), typeof(Phantom.Droid.Renderers.PhantomWebviewRenderer))]

namespace Phantom.Droid.Renderers
{
    public class PhantomWebView : Android.Webkit.WebView, InputConnectionCompat.IOnCommitContentListener
    {
        public PhantomWebView(Context context) : base(context)
        {
        }

        public PhantomWebView(Context context, IAttributeSet attrs) : base(context, attrs)
        {
        }

        public PhantomWebView(Context context, IAttributeSet attrs, int defStyleAttr) : base(context, attrs, defStyleAttr)
        {
        }

        [Obsolete]
        public PhantomWebView(Context context, IAttributeSet attrs, int defStyleAttr, bool privateBrowsing) : base(context, attrs, defStyleAttr, privateBrowsing)
        {
        }

        public PhantomWebView(Context context, IAttributeSet attrs, int defStyleAttr, int defStyleRes) : base(context, attrs, defStyleAttr, defStyleRes)
        {
        }

        protected PhantomWebView(IntPtr javaReference, JniHandleOwnership transfer) : base(javaReference, transfer)
        {
        }
    ...
    // Implemented our own Phantom webview android renderer to handle known issues in Xamarin Forms 3.3
    ...
    public class PhantomWebviewRenderer : WebViewRenderer
    {
        public PhantomWebviewRenderer(Context context) : base(context)
        {
        ...
        {
            return new PhantomWebView(Context);
        }
        ...
        protected override void OnElementChanged(ElementChangedEventArgs<Xamarin.Forms.WebView> e)
        {
            ...
            if (Control != null)
            {
                Control.SetWebViewClient(new PhantomWebViewClient(this));
            }
        }

        class PhantomWebViewClient : WebViewClient
        {
            WebNavigationResult _navigationResult = WebNavigationResult.Success;
            PhantomWebviewRenderer _renderer;

            public PhantomWebViewClient(PhantomWebviewRenderer renderer)
            {
                if (renderer == null)
